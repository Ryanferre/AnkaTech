...
export default function EditativoComponent(){
    ...
    const RemoveAtivo= (indexRemove: number)=>{
        ...
        Getmensage([<p className="text-black">Ativo deletado. Nos de a confirmacao no campo abaixo!</p>])
    }
    ...
    const RemoveAtivos= ()=>{
        const startpushAtivos= async ()=>{
            if(receiveAssets.length > 0){
             try {
                ...
                console.warn(error)
                Getmensage([<p className="text-black">Tivemos problema ao remover ativos. Desculpe...</p>])
             }
            }else{
                Getmensage([<p className="text-black">Nem um ativo encontrado!</p>])
            }
        }
        ...
        if(checked){
            ...
            Getmensage([<p className="text-black">Confirme a exclusao no campo abaixo!</p>])
        }
    }
        ...
        <section>
            <ul className="flex flex-col h-130 overflow-auto items-center w-full px-10 gap-3">
                {receiveAssets.length > 0 ? receiveAssets.map((objectReceive, index)=>(
                     <li key={index} className="grid grid-cols-4 items-center w-full px-4 h-14 border border-[#5d5d5d] rounded-[.5rem]">

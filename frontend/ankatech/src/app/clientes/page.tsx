...
export default function clientList(){
    ...
    return(
        <section className="flex flex-col py-11 pt-32 lg:pt-0 items-center px-7">
            <div className="w-full lex flex-col items-center px-7 py-8 gap-17 border border-[#5d5d5d] rounded-2xl">
                    ...
                    {userClients.length !== 0 ? userClients.map((dataclient)=>(
                       <Link href={`/SelectAtivos?id=${dataclient.id}&name=${dataclient.nome}`} key={dataclient.id}>
                          <li className="grid grid-cols-3 items-center w-full px-4 h-14 border border-[#5d5d5d] rounded-[.5rem]">
                            ...
                            <p className="text-[#5d5d5d]">{dataclient.cpf}</p>
                            <p className="text-[#5d5d5d]">{dataclient.telefone}</p>
                          </li>  
                       </Link>                      

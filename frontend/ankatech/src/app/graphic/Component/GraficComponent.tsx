...
import Assistent from '@/app/Componentes/Assistente/assistente'
...
const GraphicComponent= ()=>{
            ...
            if(nameActive == "GC=F"){
               ...
               try {
                const getinserver= await axios.get(`https://ankatech.onrender.com/getdatagraphic/${nameActive}/${typeName}`)
                    ...
                    Getmensage([<p className="text-black">Encontramos um erro ao caregar o grafico. <br/> Pedimos desculpas pelo acontecido</p>])
                    console.log(error)
                } 
            }
        }
    ...
    return(
        <section className='w-full h-full px-6 flex flex-col gap-8'>
            ...
            <Assistent assitenteInfor={topBottomPrice} assistenteComand="Você é um assistente de uma empresa que apresenta gráficos. Responda com um texto objetivo e direto, iniciando com 'O gráfico revela que ' seguido de um resumo curto do gráfico, e finalize com ' Quer analise detalhada e saber se é bom investir?'. Não mencione que é uma resposta. O texto deve ter no máximo 80 caracteres." existsButton={true} dataGraphi={DataGraphic} tipeIformation={`${nameActive} `} newDayHome={null}/>
        </section>
    )
}
